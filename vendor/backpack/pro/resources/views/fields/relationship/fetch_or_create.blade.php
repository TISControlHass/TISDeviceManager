...
<select name="{{ $field['name'] . ($field['multiple'] ? '[]' : '') }}"
    ...
    data-debug="{{ config('app.debug') }}" data-is-pivot-select="{{ var_export($field['is_pivot_select']) }}"
    data-parent-loaded-fields={{ json_encode(Basset::loaded()) }} bp-field-main-input
    @if ($activeInlineCreate) data-inline-create-route="{{ $field['inline_create']['create_route'] ?? false }}"
                ...
                data-inline-allow-create="{{ var_export($activeInlineCreate) }}" @endif
    @include('crud::fields.inc.attributes', ['default_class' => 'form-control select2_field']) @if ($field['multiple']) multiple @endif>
        ...
        <script>
                ...
                $inlineCreateButtonElement.on('click', function() {
                    ...
                    $.ajax({
                        ...
                        error: function(result) {
                            ...
                            $inlineCreateButtonElement.html($inlineCreateButtonElement.data(
                                'original-text'));
                        }
                    });
                });
            ...
            function triggerModal(element) {
                ...
                $modalSaveButton.on('click', function() {
                    ...
                    var loadingText =
                        '<i class="la la-spinner la-spin"></i>{{ trans('backpack::crud.inline_saving') }}';
            ...
            function bpFieldInitFetchOrCreateElement(element) {
                ...
                if (!element.hasClass("select2-hidden-accessible")) {
                    ...
                    element.select2({
                        ...
                        ajax: {
                            url: $dataSource,
                            dropdownParent: $isFieldInline ? $('#inline-create-dialog .modal-content') : $(document
                                .body),
                            type: $method,
                            dataType: 'json',
                            delay: $ajaxDelay,
                            data: function(params) {
                                if ($includeAllFormFields) {
                                    return {
                                        ...
                                        form: form.serializeArray(), // all other form inputs
                                        triggeredBy: {
                                            'rowNumber': element.attr('data-row-number') !== 'undefined' ? element
                                                .attr('data-row-number') - 1 : false,
                            ...
                            processResults: function(data, params) {
                                ...
                                if ($isPivotSelect) {
                                    ...
                                    let pivotsContainer = element.closest('div[data-repeatable-holder=' +
                                        containerName + ']');
                                    var selectedValues = [];

                                    pivotsContainer.children().each(function(i, container) {
                                        $(container).find('select').each(function(i, el) {
                                            if (typeof $(el).attr('data-is-pivot-select') !==
                                                'undefined' && $(el).attr('data-is-pivot-select') !=
                                                "false" && $(el).val()) {
                                ...
                                return {
                                    results: $.map(data, function(item) {
                                        ...
                                        if (selectedValues && selectedValues.some(e => e == item[
                                                $connectedEntityKeyName])) {
                                            disabled = true;
                                        }
