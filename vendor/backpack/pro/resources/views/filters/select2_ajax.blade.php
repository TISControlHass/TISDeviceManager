...
<li filter-name="{{ $filter->name }}" filter-type="{{ $filter->type }}" filter-key="{{ $filter->key }}"
    ...
    <div class="dropdown-menu p-0 ajax-select">
        <div class="form-group mb-0">
            <select id="filter_{{ $filter->key }}" name="filter_{{ $filter->name }}"
                class="form-control input-sm select2" placeholder="{{ $filter->placeholder }}"
                data-filter-key="{{ $filter->key }}" data-filter-type="select2_ajax"
                data-filter-name="{{ $filter->name }}" data-select-key="{{ $filter->options['select_key'] ?? 'id' }}"
                ...
                @if (Request::get($filter->name))
                    <option value="{{ Request::get($filter->name) }}" selected="selected">
                        {{ Request::get($filter->name . '_text') ?? 'Previous selection' }} </option>
                @endif
            </select>
        </div>
    </div>
        ...
        jQuery(document).ready(function($) {
            ...
            $('#filter_{{ $filter->key }}').each(function() {
                ...
                $(this).select2({
                        ...
                        processResults: function(data) {
                            ...
                            if (data.data) {
                                return {
                                    ...
                                    pagination: {
                                        more: typeof data.next_page_url !== 'undefined' && data
                                            .next_page_url !== null,
                                    },
                                };
                            }
                    ...
                    var val = $(this).val();
                    var val_text = $(this).select2('data')[0] ? $(this).select2('data')[0].text :
                        null;
                    ...
                    if (URI(new_url).hasQuery(filterName, true)) {
                        $('li[filter-key=' + filterKey + ']').removeClass('active').addClass(
                            'active');
                    }
