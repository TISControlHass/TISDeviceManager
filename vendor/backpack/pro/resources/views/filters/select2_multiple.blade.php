    ...
    <div class="dropdown-menu p-0">
        <div class="form-group backpack-filter mb-0">
            <select id="filter_{{ $filter->key }}" name="filter_{{ $filter->key }}"
                ...
                data-filter-key="{{ $filter->key }}" data-filter-type="select2_multiple"
                data-filter-name="{{ $filter->name }}" data-language="{{ str_replace('_', '-', app()->getLocale()) }}"
                multiple>
                @if (is_array($filter->values) && count($filter->values))
                    @foreach ($filter->values as $key => $value)
                        <option value="{{ $key }}" @if (
                            $filter->isActive() &&
                                json_decode($filter->currentValue) &&
                                array_search($key, json_decode($filter->currentValue)) !== false) selected @endif>
                            {{ $value }}
                        </option>
                    @endforeach
                @endif
                ...
                $(this).select2({
                    ...
                    if (currentElementValue.length) {
                        ...
                        if (!currentElementValue.length) {
                            ...
                            $("li[filter-key=" + filterKey + "]").find('.dropdown-menu')
                                .removeClass("show");
                        }
                    }
